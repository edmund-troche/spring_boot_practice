...
public class GreeterController extends BaseController {
    ...
    ResponseEntity<?> index(
            @RequestHeader(value = "content-language", defaultValue = "en-US")
        ...
        return ResponseEntity.ok()
                .header(
                        "content-language",
                        convertLocaleToContentLanguage(MESSAGE.getLocale().toString()))
     ...
     *
     * <p>If the locale is not `en_US` or `es`|`es_SP` then use `en_US`.
     *
     * <p>TODO: use a resource file for the per-language messages.
     */
